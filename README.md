...
- 每条线路默认开启“自动测速择优”；可取消并保存，恢复健康 IP 固定模式。手动指定 IP 优先，点击“测速并自动择优”可解除手动固定并排队重测。
- 自动只比较同国家、符合 IP 类型和服务商排除规则的可用候选；真实出口国家、类型和服务商再次验证后才测速。住宅优先不会为了速度改用机房。
- 每条线路每 15 分钟分批轮测最多 6 个（含当前节点基准，其他节点按最久未测轮换）；全机只有一个临时测速连接，不替换在用隧道。实际开始时间受全机排队影响。
- 每节点两次 HTTPS 下载，各最多 1 MiB / 10 秒，使用较慢结果；足量的超时部分下载也计入低速结果。显示的是 VPS 经住宅出口到测速站的小样本速率，含握手，并非峰值或手机/电脑到 VPS 的端到端速度。
- 自动选择近 3 小时有效实测结果中的最快合格节点，不宣称未测节点更慢；同类型提升超过 25% 才主动切换，两次自动切换至少间隔 30 分钟。手动发起择优可跳过这两个防抖限制，但仍需测得更快或更优 IP 类型。
- 测速失败不会把正常节点标为不可用；切换仍需正式连接验证，新节点连接失败优先尝试恢复原节点。真正切换会短暂重建当前线路的连接，其他线路不变。
- 低于 128 MiB 可用内存或负载偏高时暂停开始新的测速。5 条线路的理论下载上限约 240 MiB/小时（不含握手/隧道开销），实际取决于可用候选和排队；可按线路关闭自动测速。
- DNS 正向缓存每代理进程默认最多 1,024 条，遵守 DNS/CNAME TTL 且最多缓存 300 秒；失败不缓存，相同域名并发查询合并，切换 VPN 或 tun0 状态/地址变化时失效，各国缓存相互隔离。
- 新功能不会改变入站端口、节点链接、订阅或 SOCKS5 账号密码；无需重新导入客户端。
