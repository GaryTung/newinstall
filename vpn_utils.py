...
# Confirmed finer-grained IDC classifications. These overrides take priority
# over broad ISP/telecom keywords, which cannot distinguish consumer access
# from an operator's datacenter or backbone allocations.
FORCED_HOSTING_IPS = {
    "121.128.66.171",  # AS4766 / KIXS-AS-KR; Ping0 prefix-level IDC result
    "47.153.119.84",  # AS5650; Ping0 hosting result overrides IPPure residential result
...
FORCED_HOSTING_NETWORKS = (
    ipaddress.ip_network("219.100.37.0/24"),  # SoftEther OPEN/VPN public exit cluster
)

# IPPure/Ping0-style conservative classification: an address is residential
# only when its access-network identity is explicit. Unknown business/transit
# networks are treated as datacenter rather than optimistically residential.
DATACENTER_ASNS = {
    174, 6939, 8100, 9009, 13335, 14061, 14618, 15169, 16276, 16509,
    18530, 20473, 24940, 36352, 395092, 46664, 51167, 53667, 60068,
    63949, 8075,
}

DATACENTER_KEYWORDS = (
    "amazon", "aws", "azure", "cloud", "colo", "colocation", "compute",
    "contabo", "data center", "datacenter", "dedicated", "digitalocean",
    "google cloud", "hetzner", "hosting", "hostwinds", "hurricane electric",
    "isomedia", "leaseweb", "linode", "m247", "microsoft", "oracle cloud",
    "ovh", "server", "servers", "transit", "vps", "vultr",
)

RESIDENTIAL_KEYWORDS = (
    "at&t", "broadband", "cable", "centurylink", "charter", "chinanet",
    "chunghwa", "claro", "comcast", "cox communications", "fiber internet",
    "fibre internet", "frontier communications", "hinet", "home internet",
    "j:com", "kddi", "korea telecom", "mobile", "ntt", "residential",
    "singtel", "sk broadband", "softbank", "spectrum", "starhub", "telmex",
    "telefonica", "telekom", "telecom", "t-mobile", "unicom", "verizon",
    "viettel", "vodafone",
)
...
def _asn_number(value: Any) -> int:
    match = re.search(r"\bAS(\d+)\b", str(value or ""), re.IGNORECASE)
    if match:
        return _safe_int(match.group(1))
    return _safe_int(value)

def classify_ip_api_record(item: dict[str, Any]) -> tuple[str, str]:
    """Classify conservatively so false-residential results are avoided."""
    query_ip = str(item.get("query") or item.get("ip") or "").strip()
    forced_network = False
    try:
        forced_network = any(ipaddress.ip_address(query_ip) in network for network in FORCED_HOSTING_NETWORKS)
    except ValueError:
        pass
    if query_ip in FORCED_HOSTING_IPS or forced_network:
        return "hosting", "datacenter"
    if item.get("mobile"):
        return "mobile", "mobile"

    identity = " ".join(
        str(item.get(key) or "") for key in ("isp", "org", "as", "asname")
    ).lower()
    quality = "proxy" if item.get("proxy") else "normal"
    if item.get("hosting") or _asn_number(item.get("as")) in DATACENTER_ASNS:
        return "hosting", "proxy" if item.get("proxy") else "datacenter"
    if any(keyword in identity for keyword in DATACENTER_KEYWORDS):
        return "hosting", "proxy" if item.get("proxy") else "datacenter"
    if any(keyword in identity for keyword in RESIDENTIAL_KEYWORDS):
        return "residential", quality

    # IP-API's hosting=false means "not positively identified as hosting";
    # it is not proof that the address is consumer residential broadband.
    return "hosting", "unverified_datacenter"

def query_ippure_current_ip(proxy_url: str, timeout: float = 12.0) -> dict[str, Any]:
    """Query IPPure through the active tunnel; its residential flag is authoritative."""
    opener = urllib.request.build_opener(
        urllib.request.ProxyHandler({"http": proxy_url, "https": proxy_url})
    )
    request = urllib.request.Request(
        "https://my.ippure.com/v1/info",
        headers={"Accept": "application/json", "User-Agent": "vpngate-manager/2.3"},
    )
    ...
    return {
        ...
        "owner": data.get("asOrganization") or "",
        "asn": f"AS{data.get('asn')}" if data.get("asn") else "",
        "as_name": data.get("asOrganization") or "",
        "location": " ".join(
            str(data.get(key) or "") for key in ("country", "region", "city")
            if data.get(key)
        ),
        "geo_country": data.get("country") or "",
        "geo_country_short": data.get("countryCode") or "",
        "ip_type": ip_type,
        "quality": quality,
        "risk_score": _safe_int(data.get("fraudScore")),
        "classification_source": "ippure",
        "ippure_checked_at": time.time(),
    }

def cache_authoritative_ip_info(ip: str, values: dict[str, Any]) -> None:
    ip = str(ip or "").strip()
    if not ip:
        return
    with ip_cache_lock:
        cache = load_ip_cache()
        entry = dict(cache.get(ip) or {})
        for key, value in values.items():
            if value not in (None, ""):
                entry[key] = value
        entry["classification_version"] = IP_CLASSIFICATION_VERSION
...
def enrich_ip_info(nodes: list[dict[str, Any]]) -> None:
    ...
    for node in nodes:
        ...
        if (
            ip in cache
            and now - cache[ip].get("cached_at", 0) < 7 * 24 * 3600
            and cache[ip].get("classification_version") == IP_CLASSIFICATION_VERSION
        ):
            cached = cache[ip]
            ...
            node["geo_country"] = cached.get("geo_country", "")
            node["geo_country_short"] = cached.get("geo_country_short", "")
            node["classification_source"] = cached.get("classification_source", "")
            node["risk_score"] = cached.get("risk_score", 0)
            node["ippure_checked_at"] = cached.get("ippure_checked_at", 0)
        else:
        ...
        try:
            with urllib.request.urlopen(request, timeout=15) as response:
                ...
                for item in data:
                    ...
                    ip_type, quality = classify_ip_api_record(item)
                    ...
                    new_entries[query_ip] = {
                        ...
                        "ip_type": ip_type,
                        "quality": quality,
                        "geo_country": item.get("country") or "",
                        "geo_country_short": item.get("countryCode") or "",
                        "classification_source": "ip-api-conservative",
                        "risk_score": 0,
